...
def mainLoop():
    ...
    bullet_sprite = pygame.image.load(root + "\\assets\\images\\bullet.png").convert_alpha()
    bullet_sprite = scaleImage(bullet_sprite, 0.2)
    ...
    enemy_sprite = pygame.image.load(root + "\\assets\\images\\shipEnemy.png").convert_alpha()
    enemy_sprite = scaleImage(enemy_sprite, 0.5)
    enemies = []
    ...
    while True:
        ...
        if keys[pygame.K_SPACE]:
            if pygame.time.get_ticks() - last_bullet > 200:
                new_bullet = GameObject(speed=Axis(0, -20), sprite=pygame.Surface.copy(bullet_sprite))
